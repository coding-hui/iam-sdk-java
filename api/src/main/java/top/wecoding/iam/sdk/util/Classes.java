...
public class Classes {
	...
	public static Class findClassByName(String className) {
		try {
			...
			throw new IllegalArgumentException(Strings.format("Unable to load class named [{}]", className));
		}
	}

	/**
	 * Determines if the class or interface represented by this object is either the same
	 * as, or is a superclass or superinterface of, the class or interface represented by
	 ...
	 * @return the value indicating whether objects of the type can be assigned to objects
	 * of this class.
	 */
	...
	/**
	 * Gets and returns the simple name of the underlying class as given in the source
	 * code.
	 * @param obj Object instance.
	 * @return the simple name of the underlying class.
	 */
	 ...
	 * also capable of resolving nested class names in Java source style (e.g.
	 ...
	 * For it to be fed to {@code
	 * Class.getResource} instead, a leading slash would also have to be prepended to the
	 * returned value.
	...
	public static <T> T newInstance(String fqcn) {
		Class<T> clazz = findClassByName(fqcn);
	...
	public static <T> T newInstance(Class<T> clazz) {
		...
		try {
			...
			throw new InstantiationException(Strings.format("Unable to instantiate class [{}]", clazz.getName()), e);
		}
	}
	...
	public static <T> T instantiate(Constructor<T> ctor, Object... args) {
		try {
			...
			throw new InstantiationException(
					Strings.format("Unable to instantiate instance with constructor [{}]", ctor), e);
		}
	}
